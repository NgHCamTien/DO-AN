...
const CouponsList = ()=>{
  ...
  useEffect(()=>{ fetch(); },[]);
  const fetch = async ()=>{ const {data} = await axios.get('/api/coupons', { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }); setCoupons(data.data || data); };
  const del = async id => { if(!confirm('Xóa coupon?')) return; await axios.delete(`/api/coupons/${id}`, { headers: { Authorization:`Bearer ${localStorage.getItem('token')}` }}); fetch(); };
  return (
    <div className="p-4">
